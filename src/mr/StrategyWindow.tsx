 ...
 *  계열마다 다르기 때문이다(BSS IRS·국고 · 퓨처스왑 IRS·선물 · 선물 하나 —
 *  차례는 `mrcarry.LEG_NAMES` 가 정본이고 2026-09-22 에 뒤집혔다).
...
export function StrategyWindow({
   ...
   *  BSS 는 IRS·국고, 퓨처스왑은 IRS·선물, 선물 아웃라이트는 하나다. CD 는
        ...
        {!run ? (
          ...
          <>
                    ...
                    <TableBody>
                      {shownTrades.map(({ t, n }) => (
                        ...
                        <TableRow
                          ...
                          <TableCell>
                            {/* 「롱/숏」이 아니라 **다리를 적는다** — 계열마다 그 말이
                                가리키는 거래가 다르고(FUT 는 롱이 선물 매도),
                                2026-09-22 규약 뒤집기로 BSS·FSW 의 뜻도 옮겨 앉았다
                                (북의 `directionLabel` 이 플라이에서 내린 그 판단).
                                이름은 서버가 계열마다 짓는다(§16). */}
            ...
            <Text font="legal" as="span" color="fgMuted">
              ...
              {hasLegLevels
                ? ' 다리 레벨(IRS 파·국고 커브·CD 91일)은 캐리와 같은 출처예요 — (IRS − 국고) × 100 = 레벨(bp)이 줄마다 그대로 닫혀요.'
                : ''}
