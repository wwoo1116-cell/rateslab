...
export interface MrRow {
  ...
  width: number | null;
  asof: string;
  /** 오늘 **다리 레벨** [트레이더 2026-09-22 — "두개나 3개를 엮는 상품의 경우에는
   *  각각의 레벨을 표시해줄 것"].
   *
   *  엮은 값 하나만 보면 「BSS −4.8bp」가 국고가 싼 것인지 IRS 가 비싼 것인지
   *  안 보인다. 단위는 **다리 자신의 것**(금리 %)이고 엮은 값의 bp 가 아니다.
   *
   *  플라이는 `null` 이다 — `2×벨리 − 윙` 이라 「다리0 − 다리1」 규약이 거짓이고,
   *  안 맞는 분해를 실으면 그럴듯하게 틀린 수가 선다(`mrseries.combo_points`).
   *  구 백엔드는 `undefined`. */
  legs?: { name: string; v: number }[] | null;
  state: MrState;
...
export const MR_STRATEGY_DEFAULTS: MrStrategyParams = {
  ...
  entryZ: 2.0,
  exitZ: 0.5,
  /** **2.5 다** [OWNER 2026-09-09] — 3.5 에서 옮겼다.
   *
   *  프리셋은 2026-09-08 에 {3.0,3.5,4.0} → {2.5,3.0,3.5} 로 갈렸는데(커밋
   *  `74d8eb8f`) 기본값이 3.5 로 남아 초기 화면이 안 바뀌고 있었다. krw-crs
   *  레인의 운용·사전등록 기본이 2.5 이고([OWNER 2026-09-08] 「대기하는 동안
   *  돈이 빠져나간다」), 한 데스크가 두 기본값을 갖지 않게 옮긴다.
   *
   *  ⚠ **두 값이 갈려 있던 것 자체는 틀린 게 아니었다 — 프레임이 다르다.**
   *  다리 하나를 무제한 자본으로 돌리면 손절은 풀수록 낫고(실가격 2.5→3.5 가
   *  34/36 승), 증거금 예산 안에서는 조일수록 낫다(손절이 자리를 비워 막힌
   *  신호가 들어간다 — 그 레인의 `RESULT_stop_in_book_2026-09-08.md`).
   *  이 창은 무제한 프레임이므로, 이 기본값은 «이 화면에서 최적» 이라는 주장이
   *  아니라 **데스크의 기본을 따라간다**는 뜻이다.
   *
   *  ⚠ 사전등록 스크립트(`backend/scripts/mr_live_wfo.py::STOP_Z`)의 3.5 는
   *  **안 따라간다** — 그 값은 OOS 를 보기 전에 못 박은 값이라 옮기면 그
   *  전진분석이 재는 것이 없어진다(그 상수의 주석). */
  stopZ: 2.5,
  costBp: 0.5,
  notional: 1_000_000,
  entryMode: 'level',
...
export interface MrPerf {
  ...
  /** RMS 낙폭(**₩**) — 낙폭의 «깊이 × 길이».
   *
   *  ⚠ 이름이 가리키는 물건이 아니다: 원 정의(Martin & McCann)의 Ulcer Index 는
   *  **백분율 낙폭의 RMS** 라 무단위다. 이 값은 그 RMS 를 원 위에서 잰 것이라
   *  문헌값과 크기를 못 견준다. 계약과 CTA 레인이 이 이름을 쓰고 있어 남긴다 —
   *  화면과 평가층이 읽는 것은 아래 `ulcerPct` 다. */
  ulcer: number;
  /** **무단위 Ulcer**(비율) [2026-09-09] — 전고점 대비 **백분율** 낙폭의 RMS.
   *  분모는 **액면**이다 [OWNER 2026-09-09 — 「액면(원금) · 거래마다 진입일 커브」].
   *  액면을 못 세운 실행(통합 장부·선물 일부)에서는 `null` 이다 — 0 이나 원 단위
   *  값으로 대신 채우면 이 칸이 다시 두 단위를 오간다. */
  ulcerPct?: number | null;
  /** **연환산 수익률**(비율) — 액면 대비. `ulcerPct` 와 짝이고, 평가층의
   *  「정규화 안 한 수익률로 비교 금지」 규율이 여기서 산다. */
...
export interface MrBookLeg {
  ...
  label: string;
  tenor: string;
  /** 이 다리의 돈이 **어느 회계에서 나왔나** [2026-09-09]. 낱개 창의
   *  `MrStrategyRun.real` 과 같은 값이고, 다리마다 갈릴 수 있다 — 실가격 대사를
   *  못 세운 다리는 엔진 근사로 되돌아간다. `null` 은 「이 백엔드가 회계를 안
   *  말한다」이지 「근사」가 아니다(구 백엔드). */
  real?: boolean | null;
  totalPnl: number;
  maxDrawdown: number;
  sharpe: number | null;
...
export interface MrBookRun {
  ...
  /** 이 장부의 돈이 **어느 회계에서 나왔나** [2026-09-09] — 불리언 하나가
   *  아니라 «몇/몇 + 어느 다리»다.
   *
   *  다리마다 실가격/근사가 갈릴 수 있으므로 합계가 **두 회계를 더한 수**일 수
   *  있다(krw-crs 레인 실측: 격자 162칸 중 42칸이 섞임 · 화면 기본 노브에서는
   *  9/9 실가격). 「근사」로 뭉뚱그리면 8/9 와 0/9 가 같은 말이 되므로 센 수와
   *  이름을 같이 받는다. `unknown` 은 그 다리가 회계를 안 말한 것이다.
   *
   *  구 백엔드는 이 필드 자체가 없다(`undefined`) — 그때 화면은 **조용히
   *  「실가격」으로 떨어지지 않고** 「이 백엔드는 회계를 안 말한다」를 적는다. */
  accounting?: {
    real: number;
    total: number;
    approx: string[];
