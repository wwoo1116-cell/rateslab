  ...
  it('랭킹 행 뒤에 **따로** 선다 — `rows.map` 밖이다', () => {
    /* 지도의 일부로 그리면(예: rows 에 밀어 넣기) 순위가 매겨진다.
       ...
       **안**으로 밀어 넣어도 초록이었다(2026-09-02 검사). 이제 `rows.map(` 의
       ...
       ⚠ 2026-09-21 오후에 이 이름이 두 번 움직였다: 표를 행으로 가르며
       `entryRows.map` 이 됐다가, 패널을 **사실로** 가르며 다시 `rows.map` 으로
       돌아왔다(순위 패널은 25계열 전부다). 명제는 내내 그대로다 — 통합 줄은
       **집계**라 |z| 순위에 낄 물건이 아니고, 그래서 map «밖»이어야 한다. */
