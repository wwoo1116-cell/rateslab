...
7. **가로도 잰다** [OWNER 2026-08-25 — "간격이 띄엄띄엄이고 … 전체적인 사이트에서의
   ...
   - **폭은 감싸는 `<Box width={N}>` 이 준다.** 상자 없이 행에 바로 놓으면 그
     ...
     열 곳 정리]
   - ★**「채운다」가 저절로 되는 컨트롤은 없다** [2026-09-23]. 셋이 각자 다른
     이유로 안 채우고, **안 채우는 것은 상자 안에서 좁아지는 게 아니라 상자를
     뚫는다**(상자는 `overflow: visible` 이라 조용하다):
     `Select` 트리거는 값만큼만 넓다 → `DROPDOWN_STYLES` ·
     `TextInput` 은 `Field` 의 `minWidth:0` 이 없으면 고유폭(211px)으로 선다 ·
     `DateInput` 은 제 껍질에 **`min-width: 164px`** 이 박혀 있어 `Field` 로도
     안 줄고 → `.sr-datefit`.
     그리고 **숫자 칸은 컨트롤까지 민다** — `.sr-num [class*='column-']` 의
     `align-items: flex-end` 가 값뿐 아니라 `TextInput` 뿌리에도 걸린다 →
     컨트롤을 안는 줄에 `.sr-numctl`.
     ⚠ **칸에 컨트롤이 하나뿐이면 안 보인다.** 고유폭이 칸 안에 들어가 오른끝에
     붙으니 도리어 맞아 보인다. 둘이 서는 순간 드러난다(페이퍼 북 청산 칸 115px
     겹침 · 백테스트 북 날짜 두 칸 −1px · 시뮬 금통위 날짜는 입력이 32px 으로
     눌려 **글자가 잘리고 있었다**). 새 컨트롤을 칸에 더할 때는 **상자 폭과
     컨트롤 폭을 따로 재라** — 같을 것이라 믿지 말 것.
