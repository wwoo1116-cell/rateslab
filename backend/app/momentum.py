...
⚠ 2026-09-15 에 IRS 모멘텀 슬리브가 따로 동결되면서 이 데스크에 등록 북이 **둘**이
됐다(`apps/sauron-v2/docs/PREREG_sleeve_5050_2026-09-15.md`). 이 면이 비추는 것은
그중 **09-08 판**이다 — 계기가 국채선물이고 거시 다리가 네 테마 부호의 **평균**
하나다. 슬리브 쪽은 IRS 3Y·10Y 에 **테마별 부호 북 넷**이고 위험선호가 확장 평균
대비라, 같은 이름을 쓰지만 다른 북이다.

★**여기 수치를 새 정의로 갈면 안 된다.** 09-08 등록이 「판정일까지 들여다보지 말 것 ·
코드를 고치지 않는다」이므로, 이 면은 그 등록을 비추는 거울로 남는다. 두 정의가
실제로 갈리는 날이 있다(실측 2026-09-15: 통화정책 205/2,378일 · 위험선호 145/2,378일 ·
위험선호 0대비 대 확장평균 96일). 오늘은 넷 다 같은 부호라 화면이 맞아 보이지만
그건 우연이다.
...
def build_board() -> dict:
    ...
    return {
        ...
        # ★어느 «등록된 북» 인지 — 2026-09-15 에 IRS 슬리브가 따로 동결되며 이 데스크의
        # 등록 북이 둘이 됐다. 이름을 서버가 낸다: 동결일을 클라이언트가 다시 적으면
        # 두 번째 진실이 생기고, `momentum-prereg-lock` 가드가 바로 그것을 막는다.
        "registry": {
            "freeze": FREEZE,
            "instrument": "국채선물",
            "macroLeg": "네 테마 부호 평균",
            "note": ("이 면은 %s 에 동결된 등록을 비춰요. 2026-09-15 에 따로 동결된 "
                     "IRS 모멘텀 슬리브는 계기도 거시 다리 구성도 다른 «별개의 북»이에요 — "
                     "여기 숫자로 그 북을 읽지 마세요." % FREEZE),
        },
    }
