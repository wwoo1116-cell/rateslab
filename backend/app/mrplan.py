...
2. **조건은 전체 표본에서 고르고, 성과·트리거만 지난 1년이다.** 1년 창은 계열당
   거래가 한 줌이라(BSS-3Y 20건/12년) 162칸 순위가 잡음이 된다 — 「격자에서 해마다
   고르지 마라」가 이 리포의 실측이기도 하다(`docs/EVAL_LANE_STATE.md` §17-4).
...
⚠ **캐시 모양**: 계열마다 `mr-plan-{sid}` 로 디스크에 굽는다. 이 페이로드의 모양을
바꾸면 `cache.SCHEMA_VERSION` 을 올려야 한다 — 이름이 새것이라 지금은 안 올렸다.
"""
...
def build_plan(rows: list[dict], *, total: int, excluded: list[dict],
    ...
    25계열의 격자+실행이 2~3분이라 첫 요청에 다 있을 수 없다. 못 구운 것은
