...
**유니버스는 세 번 넓어졌다.** 2026-08-25 에는 본드스왑(IRS − 국고) 전 테너
...
#
# 선물 값은 **내재수익률**이다 — «IRS − 선물» 이 성립하려면 금리끼리 빼야
# 한다(차례는 2026-09-22 규약 뒤집기의 것이고, 「금리끼리」라는 요점은 그대로다). 출처는 2026-08-25 부터 **벤더 내재금리 직독**(`futures.load().implied`,
...
KIND_DEFN = {
    # ★부호 규약 **스왑 − 채권현물/국채선물** [OWNER 2026-09-22 — "BSS나 FSW같은거
    # 이제 컨벤션 바꾸는게 스왑 - 채권현물 또는 국채선물이야"]. 근거와 불변량은
    # `mrseries.points` 머리에.
    "bss": "IRS − 국고",
...
# 「롱/숏」이라고만 적으면 읽는 사람이 채권 방향으로 읽는다. 그 함정은 계열마다
# 다르다 — FUT 는 금리 계열이라 「롱」이 선물 **매도**이고, BSS·FSW 는 규약이
# `스왑 − 현물/선물` 로 뒤집힌 2026-09-22 부터 「롱」이 현물·선물 **매수**다
# (종전에는 매도였다). 그래서 이 사전은 낱말을 만들지 않고 **다리를 적는다**.
#
#   BSS = IRS − 국고  → +1 은 스프레드 확대에 거는 쪽 = IRS 페이 · 국고 매수
#                        −1 은 축소에 거는 쪽       = IRS 리시브 · 국고 매도
#   FUT = 선물 내재금리 → +1 은 금리 상승 = 선물 매도 (백테스트 북의 방향 라벨은
#                        **가격** 계열이라 부호가 반대다 — 여기는 금리다)
#   FSW = IRS − 선물내재 → 백테스트 북 `directionLabel` 의 퓨처스왑과 같은 문장
...
DIR_LEGS = {
    "bss": {"plus": {"short": "국고 매수", "legs": "IRS 페이 · 국고 매수"},
            "minus": {"short": "국고 매도", "legs": "IRS 리시브 · 국고 매도"}},
    "fut": {"plus": {"short": "선물 매도", "legs": "선물 매도"},
            "minus": {"short": "선물 매수", "legs": "선물 매수"}},
    "fsw": {"plus": {"short": "선물 매수", "legs": "IRS 페이 · 선물 매수"},
            "minus": {"short": "선물 매도", "legs": "IRS 리시브 · 선물 매도"}},
...
# BSS 의 **−1**(스프레드 축소 = `IRS − 국고` 가 내리는 쪽)이 국고를 빌려 파는
# 다리라 그 규칙에 걸린다. 선물은 대차가 필요 없으므로 FUT·FSW 는 양방향 그대로다.
#
# ⚠ 막히는 쪽이 `+1` 에서 `−1` 로 **옮겨 앉았다** [OWNER 2026-09-22, 규약 뒤집기].
# 계열 부호를 뒤집으면 같은 거래가 반대 부호로 불린다 — 실행 금지는 「어느 부호」가
# 아니라 「국고를 파는가」에 걸려 있으므로 이 사전도 같이 뒤집어야 **같은 거래**가
# 계속 막힌다. 한쪽만 뒤집으면 이 데스크가 못 하는 거래가 조용히 열린다.
#
...
def _fut_bundle() -> dict[str, dict[str, Any]]:
    ...
    for sid, (typ, col) in FSW_IRS_COL.items():
        ...
        for t, v in yields[typ]:
            ...
            if leg is None:
                continue
            # 다리 둘을 그대로 싣는다 — 화면의 «IRS − 선물 = 레벨» 이 닫히고,
            # 대사표가 다리마다 Δbp 를 잰다. 값은 %(레벨 문법) 그대로다.
            # ★차례·부호는 **스왑이 앞**이다 [OWNER 2026-09-22] — `mrseries.points`
            # 머리의 그 규약이고, BSS 와 한 규약이라야 두 상품이 같은 말을 한다.
            pts.append({"t": t, "v": round((float(leg) - v) * 100.0, 4),
                        "roll": t in rd,
                        "legs": [float(leg), v]})
