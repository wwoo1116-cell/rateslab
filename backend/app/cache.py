...
# v21 (2026-09-22): **모양이 아니라 «뜻»이 바뀐 첫 승급**이다. 청산 규칙을
# `|z| ≤ exitZ` 에서 교차(롱 z ≥ −exitZ · 숏 z ≤ +exitZ)로 고쳤다
# [OWNER — "0 sigma를 뚫고가면 청산하겠다는거야"]. 페이로드의 칸은 한 자도 안
# 바뀌었지만 **그 칸에 담기는 수가 달라진다** — 계획면 격자가 고르는 조건이
# 계열마다 갈리고(exitZ=0 칸이 전에는 불구라 격자가 피했다), 거래·손익이 전부
...
def peek(
    ...
    [2026-09-21, MR 계획면]: 25계열의 격자+실행이 약 1분이라(실측 66초) 한 요청 안에서 못
    끝내고, 라우트는 «지금까지 구워진 것» 만 내주면서 나머지를 배경에서 굽는다.
    그 라우트가 `cached()` 를 부르면 첫 요청이 그 자리에서 그 1분을 서 버린다.
