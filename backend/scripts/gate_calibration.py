...
**문턱을 안 건드린다.** 이 스크립트는 문턱이 **실물 눈금에서 어디쯤인지**만 낸다.
바꿀지 말지는 오너가 정한다.

⚠ 그리고 2026-09-10 에 실제로 오너가 바꿨다(§21) — PBO 0.20 → 0.50, 위약 신설.
**이 파일이 그 결정의 재료였다**: 허들의 4분의 3이 표본길이에서 온다는 §16-1 의
분해가 「문턱을 낮추는 것」이 답이 아님을 보였고, 그래서 낮추는 대신 구성을 바꿨다.
아래 표의 허들은 `DSR_PASS` 만 쓰므로 그 변경에 영향받지 않는다.
