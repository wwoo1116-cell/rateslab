...
| 순환이동 무작위화 검정 p값 | 0.0105 (통과) | 0.0561 (미통과) | < 0.05 |
...
국채선물 구성의 경우 213개 중 11개가 실현치를 상회하여 p값이 0.0561이다. 즉 국채선물
구성은 동일한 장부의 시점만을 임의로 이동시켜도 20회 중 1회 이상 실현 성과를
상회한다.

한 가지 유의할 점은 국채선물 구성의 p값이 측정 시점에 따라 변동한다는 것이다.
국채선물 계열은 판정 창의 종료일 이후로도 연장되는 반면 무작위화 검정은 계열 전체를
순환이동시키므로, 새로운 관측이 추가될 때마다 위약분포가 달라진다. 2026년 9월 10일
측정치는 0.0607, 9월 11일 측정치는 0.0561이며 판정은 동일하다. 이자율스왑 구성은
계열이 판정 창에서 절단되므로 이 변동이 발생하지 않는다.
